...
def init_unigram_table(word_list, freq, int train_words):
    ...
    # calculate total count for 3/4 unigram distribution
    for word in freq:
        train_words_pow += pow(freq[word], power)
    
    # word probability of 3/4 unigram distribution
    d1 = pow(freq[word_list[idx]], power) / train_words_pow;
    for i in range(table_size):
        unigram_table[i] = idx
        if (<double>i / table_size) > d1:
            # idx get enough entries in unigram_table, move to the next idx
            idx += 1
            d1 += pow(freq[word_list[idx]], power) / train_words_pow;
        if idx >= vocab_size:
            idx = vocab_size - 1;
